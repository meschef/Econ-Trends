...
colnames(trade_gs) <- c("date", "total balance", "goods balance", 
                        ...
                        "services imports")
trade_gs <- trade_gs[-c(1,14,15,16,29,30,33:47),]
...
### date formatting ###
# inserting years
date <- c(rep("2020", 12), rep("2021", 12), rep("2022",2))
date <- cbind(trade_gs[1:26,1],date)
...
trade_gs$month <- recode(trade_gs$month, "January"="01", "January (R)"= "01", "February"="02", "February (R)"= "01", "March"="03", "March (R)"= "03", "April"="04", "April (R)"="04","May"="05","May (R)"= "05", "June"="06", "June (R)"= "06","July"="07", "July (R)"= "07","August"="08", "August (R)"="08","September"="09", "September (R)" ="09", "October"="10", "October (R)"="10", "November"="11","November (R)"="11", "December"="12", "December (R)"="12")

# concatenate date
date <- paste(trade_gs$year, trade_gs$month, "01", sep = "-")
trade_gs <- cbind(date, trade_gs[,2:11])

trade_gs<- trade_gs[,-c(2)]
...
dygraph_goods <- dygraph(goods, ylab = "Billions of US Dollars", xlab = "Date") %>%
  ...
  dyAxis("y", valueRange = c(75, 335)) %>%
  dyRangeSelector(dateWindow = c(as.Date("2019-12-01"), as.Date(end_date)))
dygraph_goods
