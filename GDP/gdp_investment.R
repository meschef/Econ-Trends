...
A006RY2Q224SBEA<-fredr("A006RY2Q224SBEA", observation_start = as.Date("1990-01-01"))
investment_dt<-as.data.table(A006RY2Q224SBEA[,c(1,3)])
avg<-mean(investment_dt$value)
negative<- investment_dt %>%
  filter(value <0) 
positive<- investment_dt %>%
  filter(value>=0)
...
investment_data<-merge(positive_data, negative_data)
colnames(investment_data)<- c("positive", "negative")


avg<-mean(investment_dt$value) %>% round(digits=2)

end_date = Sys.Date() + 1095

gdp_investment<-dygraph(investment_data, xlab= "Date", ylab = "Percentage Points of GDP at Annual Rate") %>%
